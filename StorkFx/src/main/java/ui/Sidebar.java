...
public class Sidebar {
    public static VBox createsidebar(BorderPane root) {
        ...
        // 5. AUTHENTICATION SECTION (SIGN IN / LOGOUT)
        ...
        // Dynamically track login sessions
        if (SessionManager.isLoggedIn()) {
            ...
            btnAuth.setOnAction(e -> {
                // FIXED: Now safely routes layout context parameters to LoginPage setup matching logic signatures
    ...
    private static VBox createPromoCard() {
        ...
        learnMore.setOnAction(e -> {
            ...
            StackPane rootNode = (StackPane) learnMore.getScene().getRoot();
            rootNode.getChildren().add(overlay);

            closeBtn.setOnAction(ev -> rootNode.getChildren().remove(overlay));
            gotItBtn.setOnAction(ev -> rootNode.getChildren().remove(overlay));

            overlay.setOnMouseClicked(ev -> {
                if (ev.getTarget() == overlay) {
                    rootNode.getChildren().remove(overlay);
                }
            });
        });
